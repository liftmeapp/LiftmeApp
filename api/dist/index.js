...
app.post('/api/clerk-webhook', express_1.default.raw({ type: 'application/json' }), async (req, res) => {
    ...
    if (eventType === 'user.created') {
        ...
        const primaryEmailObject = email_addresses?.find((email) => email.id === primary_email_address_id);
        ...
        const userDataForDb = {
            ...
            firstName: first_name || unsafe_metadata?.firstName || 'New',
            lastName: last_name || unsafe_metadata?.lastName || 'User',
...
app.get('/api/vehicles', (0, clerk_sdk_node_1.ClerkExpressWithAuth)(), async (req, res) => {
...
app.post('/api/vehicles', (0, clerk_sdk_node_1.ClerkExpressWithAuth)(), async (req, res) => {
    const ownerId = req.auth.userId;
    if (!ownerId) {
        return res.status(401).json({ error: 'Unauthorized' });
    }
...
app.delete('/api/vehicles/:id', (0, clerk_sdk_node_1.ClerkExpressWithAuth)(), async (req, res) => {
    const ownerId = req.auth.userId;
    if (!ownerId) {
        return res.status(401).json({ error: 'Unauthorized' });
    }
...
app.get('/api/services', (0, clerk_sdk_node_1.ClerkExpressWithAuth)(), async (req, res) => {
    try {
...
app.get('/api/users/my-business', (0, clerk_sdk_node_1.ClerkExpressWithAuth)(), async (req, res) => {
    const ownerId = req.auth.userId;
    if (!ownerId) {
        return res.status(401).json({ error: 'Unauthorized' });
    }
...
app.get('/api/users/my-roles', (0, clerk_sdk_node_1.ClerkExpressWithAuth)(), async (req, res) => {
    const clerkId = req.auth.userId;
    if (!clerkId) {
        return res.status(401).json({ error: 'User not authenticated.' });
    }
...
app.get('/api/debug/database', (0, clerk_sdk_node_1.ClerkExpressWithAuth)(), async (req, res) => {
    try {
